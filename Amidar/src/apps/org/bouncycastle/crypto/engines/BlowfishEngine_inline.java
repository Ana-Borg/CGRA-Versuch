 ...
 */
public class BlowfishEngine_inline
{
        ...
        int[]   table)
    {
        ...
        for (int s = 0; s < size; s += 2)
        {
            ...
            {
                //xr ^= F(xl) ^ P[i];
		xr ^= (((S0[(xl >>> 24)] + S1[(xl >>> 16) & 0xff]) ^ S2[(xl >>> 8) & 0xff]) + S3[xl & 0xff]) ^ P[i];
                //xl ^= F(xr) ^ P[i + 1];
		xl ^= (((S0[(xr >>> 24)] + S1[(xr >>> 16) & 0xff]) ^ S2[(xr >>> 8) & 0xff]) + S3[xr & 0xff]) ^ P[i + 1];
            }

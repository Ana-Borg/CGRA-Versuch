...
public class RC6Engine_inline
{
	...
	int keylength = key.length;
        ...
        {
	    int j = i / bytesPerWord;
           L[j] = (L[j] << 8) + (key[i] & 0xff);
...
//            A = _S[i] = rotateLeft(_S[i] + A + B, 3);
	    ...
	    _S[i] = (((_S[i] + A + B) << (3 & (wordSize-1))) | ((_S[i] + A + B) >>> (wordSize - (3 & (wordSize-1)))));
	    A = _S[i];

//	     B =  L[j] = rotateLeft(L[j] + A + B, A+B);    

	    L[j] = (((L[j] + A + B) << ((A + B) & (wordSize-1))) | ((L[j] + A + B) >>> (wordSize - ((A + B) & (wordSize-1)))));
	    B = L[j];
        ...
        int     outOff)
    {
        ...
        for (int i = 1; i <= _noRounds; i++)
        {
            ...
            t = B*(2*B+1);
        //    t = rotateLeft(t,5);
	    t = ((t << (5 & (wordSize-1))) | (t >>> (wordSize - (5 & (wordSize-1)))));
            
            u = D*(2*D+1);
//            u = rotateLeft(u,5);    
            u = ((u << (5 & (wordSize-1))) | (u >>> (wordSize - (5 & (wordSize-1)))));
            A ^= t;
//            A = rotateLeft(A,u);
	    A = ((A << (u & (wordSize-1))) | (A >>> (wordSize - (u & (wordSize-1)))));
            A += _S[2*i];
            
            C ^= u;
        //    C = rotateLeft(C,t);
	    C = ((C << (t & (wordSize-1))) | (C >>> (wordSize - (t & (wordSize-1)))));
            C += _S[2*i+1];

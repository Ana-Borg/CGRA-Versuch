 ...
 *  
 */
public final class WhirlpoolDigest_inline
{
    private int BYTE_LENGTH;
    private int DIGEST_LENGTH_BYTES;
    private int ROUNDS;
    private int REDUCTION_POLYNOMIAL;
    ...
    public WhirlpoolDigest_inline()
    {
	
	this.BYTE_LENGTH = 64;
	this.DIGEST_LENGTH_BYTES = 512 / 8;
	this.ROUNDS = 10;
	this.REDUCTION_POLYNOMIAL = 0x011d; // 2^8 + 2^4 + 2^3 + 2 + 1;

	this.BITCOUNT_ARRAY_SIZE = 32;
	this._bufferPos = 0;

	this.EIGHT = new short[BITCOUNT_ARRAY_SIZE];
        this.EIGHT[BITCOUNT_ARRAY_SIZE - 1] = 8;

	this._rc = new long[ROUNDS + 1];
	this._buffer    = new byte[64];
	this._bitCount  = new short[BITCOUNT_ARRAY_SIZE];
	this._hash  = new long[8];
    	this._K = new long[8]; // the round key
    	this._L = new long[8];
    	this._block = new long[8]; // mu (buffer)
    	this._state = new long[8]; // the current "cipher" state
	this.C0 = new long[256];
        this.C1 = new long[256];
	this.C2 = new long[256];
        this.C3 = new long[256];
        this.C4 = new long[256];
        this.C5 = new long[256];
	this.C6 = new long[256];
        this.C7 = new long[256];
    ...
    // -- buffer information --
    private int BITCOUNT_ARRAY_SIZE;
    private int     _bufferPos;
    private byte[]  _buffer;//    = new byte[64];
    private short[] _bitCount;//  = new short[BITCOUNT_ARRAY_SIZE];
    
    // -- internal hash state --
    private long[] _hash, _K, _L, _block, _state;
...
//             System.arraycopy(_L, 0, _state, 0, _state.length);
	    for (int i = 0; i < 8; i++) _L[i] = _state[i];
        }

...
public class IDCT_onedim {
    ...
    public IDCT_onedim (int[] frequencies) {
    ...
    private byte[] getTransformedInt1DFast() {
        ...
        for (int i=0; i<8; i++) {
            for (int j=0; j < 8; j++) {
                ...
                int ind=i;
                for (int k=0; k<8; k++) {
                    x+= z[ind]*cos_i_k[j*8+k];
                    ind+=8;
                }
                res[j*8+i]=(byte)((x+(1<<(COSBITS-1))) / (1<<COSBITS+1));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        IDCT_onedim idct=new IDCT_onedim(input);
	byte[] t = {};

...
public class PHIInstr extends Instruction {
	...
	private void merge_stacks(SynthData data, VStack t_stack, VStack f_stack) {
		...
		while(t_stack.size() != 0 && f_stack.size() != 0) {
			...
			Instruction c1 =  I.ISTORE.create(ifinstr().addr());
			c1.branchpoint(this);
			c1.decision(true);
			Instruction c2 =  I.ISTORE.create(ifinstr().addr());
			c2.branchpoint(this);
			c2.decision(false);
			
			int lvID = data.getFreeLVID();
			
			Datum s1 = new LWriteDatum(lvID, c1, nd);
			Datum s2 = new LWriteDatum(lvID, c2, od);
			
			Instruction cl =   I.ILOAD.create(ifinstr().addr());
			Datum l = new LReadDatum(lvID, cl);

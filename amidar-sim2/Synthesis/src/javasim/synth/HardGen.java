...
public class HardGen {
	 ...
	 */
	public int MAX_INLINE_LENGTH = 1000;
	 ...
	 */
	public HardGen(MethodDescriptor[] methodTable, SynthesizedKernelDescriptor[] kernelTable, ArrayList<Invokation> invokationHistory, Integer methodIndex, int start, int stop, boolean cse, boolean inline, int maxUnrollLength, int unroll, byte[] code, CgraModel model, AliasingSpeculation aliasing, boolean constantFolding) {
		...
		for (int i = 0; i < this.code.length; i++) {
			this.code[i] = ( (short) (0xFF&code[codeOffset + i])); //TODO 
		}
		...
		codePreparation();
		lVarOffset = new Short[code.length];
...
//				System.out.println("-------------- > oi");
		...
		while(backwardJumps.size() != 0){
			...
			if((code[i]&0xFF) == 0xA7){ // GOTO
				...
				if( head > lastGoTo){ 	//found inner loop 
					...
					lastIncrease = unrollCode(unrolln, head, i); 
	...
	private int unrollCode(int unroll, int start, int stop){
		// Zu beachten:
...
//		case FREM: return Amidar.OP.FREM;
		...
		default: System.err.println(i); return Amidar.OP.ERROR;
		}
	 ...
	 * @return
	 */
	public ArrayList<Integer> getTokenSet() {
		return tokenSet;
	}
	...
	private void schedule() throws MissingOperationException{

		LPW lpw = new LPW(cdfg);

		// Schedule
		listSched = new RCListSched(cdfg, loopGraph, nrOfLocalVariables, model);
...
//		listSched.printPEutilization();
		...
		// Generate tokens
		tokenSet = listSched.tokenGeneration();
	 ...
	 * @throws SequenceNotSynthesizeableException
	 */
	public void generate() throws SequenceNotSynthesizeableException {
		generateCDFG();
		try {
			schedule();
		} catch (MissingOperationException | NotEnoughHardwareException e) {
	...
	public void printGraphs(){
		...
		listSched.draw(modName+"-schedule.sch");
	}
